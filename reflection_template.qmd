...
bay_area_long <- bay_area_childcare %>%
  pivot_longer(
    cols = contains(c("mc_", "mfcc_")),  # Any column containing these patterns
    names_to = "type_age",
    values_to = "median_price"
  )
```
...
# Calculates cost statistics and demographic medians by group
...
# REVISED to include map() family

variables_to_analyze <- c("mhi_2018", "total_pop", "households")

ca_childcare %>%
  group_by(region) %>%
  summarise(across(all_of(variables_to_analyze), 
                   ~ mean(., na.rm = TRUE), 
                   .names = "mean_{.col}"))
```
